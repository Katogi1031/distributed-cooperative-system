...
const int prey_INF = 50;          // エッジが存在しない場合の表現(無限大の距離)
...
Position PreyGetPosition(int n, int field[8][8]){
    int i, j;
    Position position;
...
// 隣接行列の作成
void AdjacementMatrix(int field[8][8]){
  ...
  for (i = 0; i < WORLD_SIZE; i++) {
    ...
    for (j = 0; j < WORLD_SIZE - 1; j++) {
      if (field[i][j] != VALUE_OF_OBSTACLE && field[i][j+1] != VALUE_OF_OBSTACLE) {
        prey_adj[from][to] = 1;
        prey_adj[to][from] = 1;
      }
  ...
  for (i = 0; i < WORLD_SIZE; i++) {
    ...
    for (j = 0; j < WORLD_SIZE - 1; j++) {
      if (field[j][i] != VALUE_OF_OBSTACLE && field[j+1][i] != VALUE_OF_OBSTACLE) {
        prey_adj[from][to] = 1;
        prey_adj[to][from] = 1;
      }
 ...
 */
void preyCreateEvalMapCorner(float evalMap[WORLD_SIZE][WORLD_SIZE], int field[8][8]){
  int i, j;
  ...
  for (i = 0; i < WORLD_SIZE; i++) {
    for (j = 0; j < WORLD_SIZE; j++) {
      if (field[i][j] == VALUE_OF_OBSTACLE) {
          /* 障害物は最低値*/
         evalMap[i][j] = 10.0;
      } else {
          /* 右上*/
 ...
 *        :各座標における評価値を保存するWORLD_SIZE*WORLD_SIZEのfloat型二次元配列
 */
void preyCreateEvalMapSurround(float evalMap[WORLD_SIZE][WORLD_SIZE], int field[8][8]){
  int i, j;
  ...
  for (i = 0; i < WORLD_SIZE; i++) {
    for (j = 0; j < WORLD_SIZE; j++) {
     if (field[i][j] == VALUE_OF_OBSTACLE) continue;

      /*上と右の確認*/
      if (field[i - 1][j] == VALUE_OF_OBSTACLE && field[i][j + 1] == VALUE_OF_OBSTACLE) {
            evalMap[i][j] += 2.0;
            evalMap[i + 1][j] += 1.0;
            evalMap[i][j - 1] += 1.0;
       }
...
void Prey(int field[16][16], int *point){
    ...
    // 障害物、角の評価
    preyCreateEvalMapSurround(evalObstacle, field);
    preyCreateEvalMapCorner(evalCorner, field);
